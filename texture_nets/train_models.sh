...
mem_score_file=$1_$2_memorability_external.csv
...
#	th test_dir.lua -image_size 256 -input_path $input_folder -model_t7 $models_folder/$file/model_10000.t7 -save_path $results_folder/$file >> log.out.txt 2>> log.err.txt
	...
	#python mem_rank.py $input_folder $texture_folder/$file $results_folder/$file $mem_score_file >> log.out.txt 2>> log.err.txt

done
python memorability_file_format.py $mem_score_file "$dataset"_external_sw"$style_weight".txt
#echo "Ploting..."
#python plot_results.py $mem_score_file $results_folder/good $results_folder/bad >> log.out.txt 2>> log.err.txt
